...
class ImageOrderManager {
    ...
    createImageWrapper(file, index) {
        ...
        imageWrapper.classList.add('relative', 'group', 'cursor-move', 'border', 'border-gray-200', 'rounded');
        ...
        removeButton.classList.add('absolute', 'top-1', 'right-1', 'bg-red-500', 'text-white', 'rounded-full', 'w-6', 'h-6', 'flex', 'items-center', 'justify-center', 'opacity-0', 'group-hover:opacity-100', 'transition', 'duration-300');
    ...
    dragStart(event) {
        const targetDiv = event.target instanceof HTMLImageElement ? event.target.closest('[data-index]') : null;
    ...
    drop(event) {
        ...
        const dropTargetIndex = parseInt(((_a = targetElement.closest('img')) === null || _a === void 0 ? void 0 : _a.dataset.index) || '-1', 10);
